...
def run_single_capacity(
    ...
    slot_feat_time_s = 0.0
    slot_score_time_s = 0.0
    slot_update_time_s = 0.0
    ...
    while True:
        ...
        features = _build_feature_vector(feature_set, gaps, history_feats)
        feat_time_s = time.perf_counter() - feat_start
        slot_feat_time_s += feat_time_s
        if hasattr(features, "nbytes"):
            slot_feature_bytes += int(features.nbytes)
            if int(features.nbytes) > max_feature_bytes:
                max_feature_bytes = int(features.nbytes)
        ...
        if global_idx >= warmup_requests:
            ...
            if cache.access(obj_id):
                ...
                slot_score_time_s += time.perf_counter() - score_start
                if y_hat == 1:
                    cache.insert(obj_id)
            pbar.update(1)
        ...
        if slot_req_count >= slot_size:
            ...
            slot_update_time_s += time.perf_counter() - update_start

            miss_candidates = int(len(slot_stats))
            slot_num = slot_index + 1
            phase = "warmup" if slot_num <= warmup_slots else "cache"
            slot_time_s = slot_feat_time_s + slot_score_time_s + slot_update_time_s
            total_feat_time_s += slot_feat_time_s
            total_score_time_s += slot_score_time_s
            total_update_time_s += slot_update_time_s
            ...
            if slot_log_file:
                slot_payload = {
                    ...
                    "slot_requests": slot_size,
                    "time_feat_s": float(slot_feat_time_s),
                    "time_score_s": float(slot_score_time_s),
                    "time_update_s": float(slot_update_time_s),
                    "time_slot_s": float(slot_time_s),
                    "feature_bytes": int(slot_feature_bytes),
                    "num_features": int(num_features),
                }
                slot_log_file.write(json.dumps(slot_payload) + "\n")
            ...
            if slot_index == warmup_slots:
                ...
                slot_update_time_s += time.perf_counter() - rebuild_start
    ...
    summary_metrics = {
        ...
        "rss_max_kb": float(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss),
        "rss_max_mb": float(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss) / 1024.0,
        "num_features": int(num_features),
    }
...
def run_experiment(
    ...
    if model_name is None:
        model_name = f"gdbt_{feature_set}_overhead"
...
def main() -> None:
    ...
    run_experiment(
        ...
        cache_size_objects=args.cache_size_objects,
