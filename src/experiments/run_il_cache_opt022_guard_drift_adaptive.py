# src/experiments/run_il_cache_opt022_guard_drift_adaptive.py
...
# Drift + guard (adaptive) configuration
DRIFT_GAIN = 1.5
DRIFT_ALPHA_FLOOR_MULT = 0.7
# Drift-aware gating (only increase budget when quality is good)
DRIFT_GUARD_PRECISION_MIN = 0.06
DRIFT_GUARD_SPREAD_MIN = 0.01
DRIFT_BAD_MULT = 1.0
...
# Adaptive score gate (% of candidates kept)
SCORE_GATE_TOP_PERCENT = 0.05
SCORE_GATE_USE_ADAPTIVE = True
SCORE_GATE_MIN_PERCENT = 0.02
SCORE_GATE_MAX_PERCENT = 0.20
SCORE_GATE_SPREAD_LOW = 0.005
SCORE_GATE_SPREAD_HIGH = 0.03
FILL_RATIO = 0.9
FILL_RATE = 0.05
...
def run_single_capacity(
    ...
    def _finalize_slot() -> None:
        ...
        if is_cache_slot:
            ...
            drift_gate_ok = True
            if (precision_eff is not None) and (score_spread is not None):
                drift_gate_ok = (
                    (precision_eff >= DRIFT_GUARD_PRECISION_MIN)
                    and (score_spread >= DRIFT_GUARD_SPREAD_MIN)
                )
            drift_norm_eff = drift_norm if drift_gate_ok else 0.0
            drift_norm_p = drift_norm_eff ** max(DRIFT_NORM_POWER, 0.0)
            ...
            if not drift_gate_ok and DRIFT_BAD_MULT != 1.0:
                alpha_base *= DRIFT_BAD_MULT
        ...
        if miss_candidates > 0:
            ...
            if SCORE_GATE_USE_ADAPTIVE:
                spread_ref = score_spread_ema if score_spread_ema is not None else score_spread
                if spread_ref is None:
                    score_gate_percent = SCORE_GATE_MIN_PERCENT
                else:
                    x = (spread_ref - SCORE_GATE_SPREAD_LOW) / max(
                        SCORE_GATE_SPREAD_HIGH - SCORE_GATE_SPREAD_LOW, SCORE_SPREAD_EPS
                    )
                    x = max(0.0, min(1.0, x))
                    score_gate_percent = SCORE_GATE_MIN_PERCENT + x * (
                        SCORE_GATE_MAX_PERCENT - SCORE_GATE_MIN_PERCENT
                    )
            else:
                score_gate_percent = SCORE_GATE_TOP_PERCENT
        ...
        if miss_candidates > 0:
            gate_percent = score_gate_percent if SCORE_GATE_USE_ADAPTIVE else SCORE_GATE_TOP_PERCENT
        ...
        slot_log = {
            ...
            "score_gate_percent": score_gate_percent,
...
def run_experiment(
    ...
    print(f"Drift guard min   : prec>={DRIFT_GUARD_PRECISION_MIN}, spread>={DRIFT_GUARD_SPREAD_MIN}")
    print(f"Gate adaptive     : {SCORE_GATE_USE_ADAPTIVE} ({SCORE_GATE_MIN_PERCENT}–{SCORE_GATE_MAX_PERCENT})")
    ...
    for capacity in capacities_objects:
        ...
        summary_payload = {
            ...
            "drift_guard_precision_min": DRIFT_GUARD_PRECISION_MIN,
            "drift_guard_spread_min": DRIFT_GUARD_SPREAD_MIN,
            "score_gate_use_adaptive": SCORE_GATE_USE_ADAPTIVE,
            "score_gate_min_percent": SCORE_GATE_MIN_PERCENT,
            "score_gate_max_percent": SCORE_GATE_MAX_PERCENT,
            "score_gate_spread_low": SCORE_GATE_SPREAD_LOW,
            "score_gate_spread_high": SCORE_GATE_SPREAD_HIGH,
