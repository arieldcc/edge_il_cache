...
DRIFT_ALPHA_MAX = 0.2
DRIFT_WEIGHT_MISS = 0.5
DRIFT_WEIGHT_JSD = 0.5
...
def run_single_capacity(
    ...
    prev_freq_hist: Optional[np.ndarray] = None
    drift_score_sum = 0.0
    miss_rate_sum = 0.0
    jsd_sum = 0.0
    alpha_sum = 0.0
    ...
    def _finalize_slot() -> None:
        ...
        if is_cache_slot:
            ...
            if top_k > 0:
                ...
                top_freqs = [int(row["freq"]) for row in D_t[:top_k]]
                freq_hist = _slot_freq_hist(slot_stats, FREQ_HIST_BINS, freq_values=top_freqs)
            else:
                freq_hist = _slot_freq_hist(slot_stats, FREQ_HIST_BINS)
            if prev_freq_hist is not None:
                jsd_freq = _compute_jsd(prev_freq_hist, freq_hist)
            prev_freq_hist = freq_hist
            ...
            if drift_scaled < 0.0:
                ...
                drift_scaled = 1.0
            alpha_base = admission_capacity_alpha * (1.0 + drift_scaled)
