...
DRIFT_METHOD = "jsd"  # "jsd", "adwin", "page_hinkley"
...
# DRIFT-ONLY: keep drift contribution
DRIFT_GAIN = 1.0
DRIFT_ALPHA_FLOOR_MULT = 0.7
# DRIFT-ONLY: disable score gate by allowing full candidate set
SCORE_GATE_TOP_PERCENT = 1.0
FILL_RATIO = 0.9
FILL_RATE = 0.05
# DRIFT-ONLY: disable miss-pressure and score-quality modulation
PRESSURE_MISS_GAMMA = 0.0
SCORE_SPREAD_Q = 0.9
...
# DRIFT-ONLY: disable precision-based scaling
...
class ADWINDetector:
    """
    ADWIN sederhana untuk scalar stream (mis. miss-rate).
    Deteksi drift jika beda mean dua subwindow melewati bound Hoeffding.
    """

    def __init__(self, delta: float = 0.002, min_window: int = 10) -> None:
    ...
    def update(self, value: float) -> Tuple[float, bool]:
        ...
        n = len(self.window)
        if n < max(self.min_window, 2):
            return 0.0, False

        max_ratio = 0.0
        drift = False
        # cek semua split; untuk slot count kecil, ini cukup cepat
        for i in range(1, n):
            n0 = i
            n1 = n - i
            ...
            m0 = sum(w0) / n0
            m1 = sum(w1) / n1
            eps = math.sqrt(0.5 * math.log(2.0 / self.delta) * (1.0 / n0 + 1.0 / n1))
            if eps <= 0:
                continue
            ratio = abs(m0 - m1) / eps
            if ratio > max_ratio:
            ...
            if ratio > 1.0:
                drift = True
                # buang bagian lama (ADWIN shrink)
                self.window = w1
                break

        raw_score = min(1.0, max_ratio)
        return raw_score, drift


class PageHinkleyDetector:
    """
    Page-Hinkley untuk perubahan mean.
    """

    def __init__(self, delta: float = 0.005, lambd: float = 0.3) -> None:
        self.delta = float(delta)
    ...
    def update(self, value: float) -> Tuple[float, bool]:
        ...
        # PH cumulative sum of deviations
        self.cum_sum += x - self.mean - self.delta
        if self.cum_sum < self.min_cum:
            self.min_cum = self.cum_sum
        ph_stat = self.cum_sum - self.min_cum
        drift = ph_stat > self.lambd
        raw_score = 0.0
        if self.lambd > 0:
            raw_score = min(1.0, max(0.0, ph_stat / self.lambd))
        return raw_score, drift
...
def run_single_capacity(
    ...
    - Jika budget_schedule diberikan, nilai admit_budget per cache slot
      di-override (tetap di-clamp oleh miss_candidates).
    """
    ...
    drift_method = (drift_method or DRIFT_METHOD).lower()
    if drift_method not in ("jsd", "adwin", "page_hinkley"):
    ...
    adwin = ADWINDetector(delta=ADWIN_DELTA) if drift_method == "adwin" else None
    page_hinkley = (
        PageHinkleyDetector(delta=PH_DELTA, lambd=PH_LAMBDA)
        if drift_method == "page_hinkley"
        else None
    )
    ...
    def _finalize_slot() -> None:
        ...
        if is_cache_slot:
            ...
            if drift_method == "jsd":
                ...
                drift_raw, _ = adwin.update(miss_rate)
            elif drift_method == "page_hinkley":
                assert page_hinkley is not None
                drift_raw, _ = page_hinkley.update(miss_rate)
        ...
        if is_cache_slot:
            ...
            if drift_method == "jsd":
                jsd_sum += float(jsd_freq or 0.0)
                overlap_sum += float(overlap_w or 0.0)
        ...
        if is_cache_slot and budget_schedule is not None:
            schedule_idx = total_cache_slots - 1
            if 0 <= schedule_idx < len(budget_schedule):
                admit_budget = int(budget_schedule[schedule_idx])
        if miss_candidates <= 0:
            admit_budget = 0
    ...
    summary_metrics = {
        ...
        "drift_method": drift_method,
        "jsd_freq_avg": (
            float(jsd_sum / total_cache_slots)
            if (total_cache_slots > 0 and drift_method == "jsd")
            else None
        ),
        "topk_overlap_w_avg": (
            float(overlap_sum / total_cache_slots)
            if (total_cache_slots > 0 and drift_method == "jsd")
            else None
        ),
        "drift_raw_avg": (
