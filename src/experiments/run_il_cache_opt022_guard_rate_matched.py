...
RATE_MATCH_DEFAULT_MODEL = "ilnse_A2_guard_only"
...
def _latest_group_summary_path(results_root: str, dataset_name: str, model_name: str) -> str:
    dataset_dir = os.path.join(results_root, dataset_name)
    pattern = f"_summary_{model_name}_all_sizes.json"
    files = []
    ...
    if not files:
        raise FileNotFoundError(
            f"No summary for dataset={dataset_name} model={model_name} in {dataset_dir}"
        )
    ...
    files = sorted(files, key=_run_id)
    return files[-1]


def _load_rate_match_targets(
    results_root: str,
    dataset_name: str,
    model_name: str,
    capacity: int,
) -> Tuple[Dict[int, int], str]:
    summary_path = _latest_group_summary_path(results_root, dataset_name, model_name)
    run_id = os.path.basename(summary_path).split("_", 1)[0]
    ...
    if not os.path.exists(jsonl_path):
        raise FileNotFoundError(f"Missing rate-match log: {jsonl_path}")
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            targets[slot_idx] = int(row.get("admit_selected") or 0)
...
def run_single_capacity(
    ...
    capacity_objects: int,
    feature_set: str,
    rate_match_targets: Optional[Dict[int, int]] = None,
    ...
    rate_match_applied_slots = 0
    rate_match_target_total = 0
    ...
    def _finalize_slot() -> None:
        ...
        nonlocal rate_match_applied_slots
        nonlocal rate_match_target_total
        ...
        rate_match_applied = False
        if rate_match_targets is not None and is_cache_slot:
            rate_match_target = rate_match_targets.get(slot_num)
            if rate_match_target is not None:
                rate_match_applied = True
                if miss_candidates <= 0:
                    admit_budget = 0
                else:
                    admit_budget = min(int(rate_match_target), miss_candidates)
        if rate_match_applied:
            rate_match_applied_slots += 1
            rate_match_target_total += int(rate_match_target or 0)
        ...
        slot_log = {
            ...
            "rate_match_target": rate_match_target,
            "rate_match_applied": rate_match_applied,
    ...
    summary_metrics = {
        ...
        "rate_match_applied_slots": rate_match_applied_slots,
        "rate_match_target_total": rate_match_target_total,
...
def run_experiment(
    ...
    rate_match_model: Optional[str] = RATE_MATCH_DEFAULT_MODEL,
    ...
    if rate_match_model:
        print(f"Rate-match model  : {rate_match_model}")
    ...
    for capacity in capacities_objects:
        ...
        rate_match_targets = None
        rate_match_run_id = None
        if rate_match_model:
            rate_match_targets, rate_match_run_id = _load_rate_match_targets(
                results_root,
                dataset_name,
                rate_match_model,
                capacity,
            ...
            print(
                f"[RATE-MATCH] source={rate_match_model} run={rate_match_run_id} "
                f"capacity={capacity} targets={len(rate_match_targets)}"
            )
        ...
        stats, summary_metrics = run_single_capacity(
            ...
            feature_set=feature_set,
            rate_match_targets=rate_match_targets,
        ...
        summary_payload = {
            ...
            "model": model_name,
            "feature_set": feature_set,
            "rate_match_model": rate_match_model,
            "rate_match_run_id": rate_match_run_id,
