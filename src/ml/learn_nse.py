...
class LearnNSE:
    ...
    def update_slot(self, dataset: List[Dict]) -> Dict[str, Any]:
        """
            ...
            slot_info.update(
                {
                    ...
                    "beta_bars": [],
                    "weights": [],
                    "timing": {"total_s": perf_counter() - t_start},
                    "per_learner_timing": [],
                }
            )
            return slot_info
        ...
        t_prepare_xy = perf_counter() - t_prepare_start
        n_samples = X.shape[0]
        ...
        train_time_s += perf_counter() - t_train

        t_pred_new = perf_counter()
        y_hat = new_model.predict(X)
        pred_time_new = perf_counter() - t_pred_new
        ...
        epsilons: List[float] = []
        per_learner_timing: List[Dict[str, Any]] = []
        for info in self.learners:
            t_pred = perf_counter()
            y_hat = info.model.predict(X)
            pred_time = perf_counter() - t_pred
            t_eps = perf_counter()
            err = float(np.average((y_hat != y).astype(float), weights=F))
            eps_time = perf_counter() - t_eps
        ...
        # snapshot SEBELUM pruning (21 learner jika baru ditambah)
        learner_rows_before_prune = self._build_learner_rows(t, epsilons, beta_bars, weights)
        ...
        learner_rows_after_prune = self._build_learner_rows(t, epsilons, beta_bars, weights)
        # print("AFTER PRUNE :", len(self.learners), len(self._weights), len(weights), prune_info)

        num_learners_after = self.num_learners()

        # 9) Akurasi ensemble setelah update
        if self.learners:
        ...
        slot_info.update(
            {
                ...
                "prune": prune_info,
                "learners_before_prune": learner_rows_before_prune,
                "learners_after_prune": learner_rows_after_prune,
                # --- DEBUG barunya di bawah ini ---
                "learner_created_at": [info.created_at_slot for info in self.learners],
                "learner_age": [t - info.created_at_slot for info in self.learners],
                "timing": {
                    "total_s": t_total,
                    "prepare_xy_s": t_prepare_xy,
                    "train_new_learner_s": train_time_s,
                    "predict_new_learner_s": pred_time_new,
                },
                "per_learner_timing": per_learner_timing,
            }
        )
        return slot_info
    ...
    def _build_learner_rows(
        ...
        rows = []
        # diasumsikan list masih aligned dengan self.learners PADA SAAT dipanggil
        for info, eps, bb, w in zip(self.learners, epsilons, beta_bars, weights):
            beta_t = info.beta_history.get(t, None)
            rows.append({
                "id": info.learner_id,
                "created_at": info.created_at_slot,
                "age": t - info.created_at_slot,
                "epsilon_t": float(eps),
                "beta_t": float(beta_t) if beta_t is not None else None,
                "beta_bar_t": float(bb),
                "weight_t": float(w),
            })
        return rows
...
class CacheAwareLearnNSE(LearnNSE):
    ...
    # ---------------------------------------------------------------------
    @staticmethod
    def _cls1_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, Any]:
        """Metrik khusus kelas-1 (positif) untuk mendeteksi collapse."""
    ...
    def update_slot(self, dataset: List[Dict]) -> Dict[str, Any]:
        ...
        Update per slot (struktur Learn++.NSE tetap):
          ...
          9) Audit (sum_F_pos/neg, pred_pos_after_update, collapse flag)
        """
        slot_info: Dict[str, Any] = {}
            ...
            slot_info.update(
                {
                    ...
                    "beta_bars": [],
                    "weights": [],
                    "cls1_metrics_before_update": None,
                    "cls1_metrics_after_update": None,
                    "sum_F_pos": None,
                    "sum_F_neg": None,
                    "ratio_F_pos_neg": None,
                    "pred_pos_after_update": None,
                    "support_pos": None,
                    "collapse_pred_pos_0": None,
                    "timing": {"total_s": perf_counter() - t_start},
                    "per_learner_timing": [],
                }
            )
        ...
        if self.learners:
            ...
            cls1_before = self._cls1_metrics(y, ensemble_pred_before)
        ...
        balance_info = {
            "balance_ratio_raw": None,
            "balance_ratio_smoothed": None,
            "balance_ratio_clamped": None,
            "balance_scale_pos": None,
            "balance_scale_neg": None,
        }
        if self.class_mass_balance and (w_pos > 0.0) and (w_neg > 0.0):
            ...
            balance_info.update(
                {
                    "balance_ratio_raw": float(ratio_raw),
                    "balance_ratio_smoothed": float(self._ema_balance_ratio),
                    "balance_ratio_clamped": float(ratio),
                    "balance_scale_pos": float(scale_pos),
                    "balance_scale_neg": float(scale_neg),
                }
            )
        ...
        # audit mass sesudah normalisasi
        sum_F_pos = float(F[y == 1].sum())
        sum_F_neg = float(F[y == 0].sum())
        ratio_F_pos_neg = sum_F_pos / (sum_F_neg + 1e-12)
        ...
        ensemble_pred_after = self._predict_batch(X)
        cls1_after = self._cls1_metrics(y, ensemble_pred_after)
        acc_after = float((ensemble_pred_after == y).mean())

        pred_pos_after = int(cls1_after["pred_pos"])
        support_pos = int(cls1_after["support_pos"])
        collapse_pred_pos_0 = bool((support_pos > 0) and (pred_pos_after == 0))
        ...
        slot_info.update(
            {
                ...
                "learner_created_at": [info.created_at_slot for info in self.learners],
                "learner_age": [t - info.created_at_slot for info in self.learners],
                "cls1_metrics_before_update": cls1_before,
                "cls1_metrics_after_update": cls1_after,
                # audit
                "sum_F_pos": sum_F_pos,
                "sum_F_neg": sum_F_neg,
                "ratio_F_pos_neg": ratio_F_pos_neg,
                "pred_pos_after_update": pred_pos_after,
                "support_pos": support_pos,
                "collapse_pred_pos_0": collapse_pred_pos_0,
                # audit freq-adaptive
                "lambda_eff": float(lambda_eff),
                "freq_norm_mean": float(mean_norm),
                "freq_norm_cv": float(cv_norm),
                "freq_factor_max": float(freq_factor.max()) if n_samples > 0 else None,
                # audit balance
                "balance_ratio_raw": balance_info["balance_ratio_raw"],
                "balance_ratio_smoothed": balance_info["balance_ratio_smoothed"],
                "balance_ratio_clamped": balance_info["balance_ratio_clamped"],
                "balance_scale_pos": balance_info["balance_scale_pos"],
                "balance_scale_neg": balance_info["balance_scale_neg"],
                "timing": {
                    "total_s": t_total,
                    "prepare_xy_s": t_prepare_xy,
                    "train_new_learner_s": train_time_s,
