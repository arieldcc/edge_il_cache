...
# Gate hanya aktif jika kualitas ranking dianggap buruk (quality_mult < threshold).
# quality_mult = score_spread / score_spread_ema, jadi 1.0 berarti setara rata-rata historis.
SCORE_QUALITY_GOOD_THRESHOLD = 1.0
SCORE_GATE_TOP_PERCENT_MAX = 0.2
SCORE_GATE_DRIFT_GAIN = 0.1
DRIFT_EXTRA_ALPHA_SCALE = 0.5
ADMISSION_PRECISION_TARGET = 0.12
ADMISSION_PRECISION_SENSITIVITY = 1.0
CAPACITY_ALPHA_SCALE_MIN = 0.4
# Variant switches (for ablations)
ENABLE_DRIFT = True
ENABLE_GUARDRAILS = True
FIXED_BUDGET_ALPHA: float | None = None  # if set, use constant alpha and skip drift/guardrails
GUARD_MODE = "conditional_gate"  # conditional_gate | drift_aware_gate | dual_budget | drift_when_good
GUARD_MODE_OPTIONS = (
    "conditional_gate",
    "drift_aware_gate",
    "dual_budget",
...
def run_single_capacity(
    ...
    enable_guardrails: bool = ENABLE_GUARDRAILS,
    fixed_budget_alpha: float | None = FIXED_BUDGET_ALPHA,
    ...
    """
    if guard_mode not in GUARD_MODE_OPTIONS:
    ...
    slot_cache_hits = 0
    slot_cache_misses = 0
    slot_cache_inserts = 0
    slot_cache_evictions = 0
    ...
    score_gate_k_sum = 0
    score_gate_percent_sum = 0.0
    score_gate_percent_count = 0
    ...
    def _apply_pending_admits(next_slot_num: int) -> None:
        ...
        nonlocal slot_cache_inserts
        nonlocal slot_cache_evictions
        nonlocal total_cache_inserts
        nonlocal total_cache_evictions
        ...
        for obj_id in applied_admit_set:
            inserted, evicted = cache.insert_with_eviction(obj_id)
            if inserted:
                slot_cache_inserts += 1
                total_cache_inserts += 1
            if evicted is not None:
                slot_cache_evictions += 1
                total_cache_evictions += 1
    ...
    def _finalize_slot() -> None:
        ...
        nonlocal score_gate_percent_sum
        nonlocal score_gate_percent_count
        ...
        nonlocal quality_good_slots
        nonlocal quality_bad_slots
        ...
        quality_good = None
        score_spread = 0.0
        apply_drift = enable_drift and (fixed_budget_alpha is None)
        apply_guardrails = enable_guardrails and (fixed_budget_alpha is None)
        ...
        if is_cache_slot:
            ...
            if apply_guardrails and len(cache) < fill_threshold:
                fill_phase = True
        ...
        if miss_candidates > 0:
            ...
            quality_good = quality_mult >= SCORE_QUALITY_GOOD_THRESHOLD
            if is_cache_slot and quality_good is not None:
                if quality_good:
                    quality_good_slots += 1
                else:
                    quality_bad_slots += 1
        ...
        # hitung alpha setelah quality_mult diketahui
        alpha_for_slot_base = admission_capacity_alpha
        alpha_for_slot = admission_capacity_alpha
        if is_cache_slot:
            apply_drift_effective = apply_drift
            if guard_mode == "drift_when_good" and quality_good is False:
                apply_drift_effective = False

            alpha_base = admission_capacity_alpha * capacity_scale
            alpha_drift = alpha_base
            if apply_drift_effective:
                alpha_drift = admission_capacity_alpha * (1.0 + DRIFT_GAIN * drift_norm) * capacity_scale
            if fixed_budget_alpha is not None:
                alpha_base = float(fixed_budget_alpha)
                alpha_drift = float(fixed_budget_alpha)
            if alpha_base < DRIFT_ALPHA_MIN:
                alpha_base = DRIFT_ALPHA_MIN
            elif alpha_base > DRIFT_ALPHA_MAX:
                alpha_base = DRIFT_ALPHA_MAX
            if alpha_drift < DRIFT_ALPHA_MIN:
                alpha_drift = DRIFT_ALPHA_MIN
            elif alpha_drift > DRIFT_ALPHA_MAX:
                alpha_drift = DRIFT_ALPHA_MAX

            precision_adjust = 1.0
            guard_precision = precision_eff if precision_eff is not None else admit_precision
            if apply_guardrails and guard_precision is not None:
                precision_delta = (
            ...
            alpha_candidate_base = alpha_base * precision_adjust
            alpha_candidate_drift = alpha_drift * precision_adjust
            if apply_guardrails:
                alpha_floor_base = alpha_base * DRIFT_ALPHA_FLOOR_MULT
                alpha_for_slot_base = (
                    alpha_candidate_base if alpha_candidate_base >= alpha_floor_base else alpha_floor_base
                )
                alpha_floor_drift = alpha_drift * DRIFT_ALPHA_FLOOR_MULT
                alpha_for_slot = (
                    alpha_candidate_drift if alpha_candidate_drift >= alpha_floor_drift else alpha_floor_drift
                )
            else:
                alpha_for_slot_base = alpha_candidate_base
                alpha_for_slot = alpha_candidate_drift
            if alpha_for_slot_base < DRIFT_ALPHA_MIN:
                alpha_for_slot_base = DRIFT_ALPHA_MIN
            elif alpha_for_slot_base > DRIFT_ALPHA_MAX:
                alpha_for_slot_base = DRIFT_ALPHA_MAX
        ...
        if is_cache_slot:
            pressure_gamma = PRESSURE_MISS_GAMMA
            if capacity_scale < 1.0:
                pressure_gamma *= 1.0 + (1.0 - capacity_scale)
            quality_for_budget = quality_mult if apply_guardrails else 1.0
            if fixed_budget_alpha is not None:
                pressure_mult = 1.0
            else:
                pressure_mult = (1.0 + pressure_gamma * miss_rate) * quality_for_budget
        ...
        admit_budget_base = 0
        admit_budget_extra = 0
        if guard_mode == "dual_budget" and is_cache_slot:
            admit_budget_base = _compute_admission_budget(
                slot_cache_misses,
                capacity_objects,
                alpha_for_slot_base,
                pressure_mult,
            )
            extra_alpha = max(alpha_for_slot - alpha_for_slot_base, 0.0)
            extra_alpha *= DRIFT_EXTRA_ALPHA_SCALE
            admit_budget_extra = _compute_admission_budget(
                slot_cache_misses,
                capacity_objects,
                extra_alpha,
                pressure_mult,
            )
            admit_budget = admit_budget_base + admit_budget_extra
        else:
        ...
        gate_percent = SCORE_GATE_TOP_PERCENT
        if guard_mode == "drift_aware_gate":
            if quality_good is True:
                gate_percent = SCORE_GATE_TOP_PERCENT + SCORE_GATE_DRIFT_GAIN * drift_norm
            else:
                gate_percent = SCORE_GATE_TOP_PERCENT
            if gate_percent > SCORE_GATE_TOP_PERCENT_MAX:
                gate_percent = SCORE_GATE_TOP_PERCENT_MAX
        if miss_candidates > 0:
            score_gate_k = max(1, int(math.ceil(gate_percent * miss_candidates)))
            score_gate_k_sum += score_gate_k
            score_gate_percent_sum += gate_percent
            score_gate_percent_count += 1
            apply_gate = apply_guardrails and not fill_phase
            if guard_mode in ("conditional_gate", "drift_when_good", "dual_budget"):
                apply_gate = apply_gate and (quality_good is False)
            if apply_gate and admit_budget > score_gate_k:
                if guard_mode == "dual_budget":
                    admit_budget = max(admit_budget_base, score_gate_k)
                else:
        ...
        slot_log = {
            ...
            "slot_cache_inserts": slot_cache_inserts,
            "slot_cache_evictions": slot_cache_evictions,
            "slot_hit_ratio": (
                current_slot_hr
            ),
            ...
            "quality_threshold": SCORE_QUALITY_GOOD_THRESHOLD,
            ...
            "score_gate_percent": gate_percent,
            "guard_mode": guard_mode,
            "admit_budget": admit_budget,
            "admit_budget_base": admit_budget_base,
            "admit_budget_extra": admit_budget_extra,
    ...
    summary_metrics = {
        ...
        "score_gate_percent_avg": (
            float(score_gate_percent_sum / score_gate_percent_count)
            if score_gate_percent_count > 0
            else None
        ),
        ...
        "quality_good_slots": quality_good_slots,
        "quality_bad_slots": quality_bad_slots,
        "quality_good_ratio": (
            float(quality_good_slots / (quality_good_slots + quality_bad_slots))
            if (quality_good_slots + quality_bad_slots) > 0
            else None
        ),
        ...
        "avg_score_time_s": (
            float(total_score_time_s / total_score_slots)
            if total_score_slots > 0
            else None
        ),
        "score_time_total_s": float(total_score_time_s),
        "cache_inserts_total": total_cache_inserts,
        "cache_evictions_total": total_cache_evictions,
        "cache_inserts_avg": (
            float(total_cache_inserts / total_cache_slots)
            if total_cache_slots > 0
            else None
        ),
        "cache_evictions_avg": (
            float(total_cache_evictions / total_cache_slots)
            if total_cache_slots > 0
            else None
        ),
...
def run_experiment(
    ...
    # Accept both dict config and DatasetConfig dataclass
    if not isinstance(ds_cfg, dict):
        try:
            ...
            elif hasattr(ds_cfg, "__dict__"):
                ds_cfg = ds_cfg.__dict__
        except Exception:
            pass
    if not isinstance(ds_cfg, dict):
        raise TypeError("ds_cfg must be a dict-like config or DatasetConfig")
    if feature_set not in FEATURE_SETS:
        raise ValueError(f"Unknown feature_set: {feature_set}")
    if guard_mode not in GUARD_MODE_OPTIONS:
        raise ValueError(f"Unknown guard_mode: {guard_mode}")
    ...
    print(f"Enable drift      : {enable_drift}")
    print(f"Enable guardrails : {enable_guardrails}")
    print(f"Guard mode        : {guard_mode}")
    if fixed_budget_alpha is not None:
        print(f"Fixed budget alpha: {fixed_budget_alpha}")
    ...
    print(f"Quality good thr : {SCORE_QUALITY_GOOD_THRESHOLD}")
    ...
    for capacity in capacities_objects:
        ...
        stats, summary_metrics = run_single_capacity(
            ...
            fixed_budget_alpha=fixed_budget_alpha,
            guard_mode=guard_mode,
        )
